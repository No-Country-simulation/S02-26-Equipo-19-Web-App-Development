...
import { useReports } from "../../hooks/caregivers/useReports";
import { usePatients } from "../../hooks/caregivers/usePatients";
...
const Caregivers = () => {
    ...
    //     },
    // ]);
    const patientsFronApi = usePatients(1);
    ...
    // };
    useEffect(() => {
        if (patientsFronApi.length && !activeId) {
            setActiveId(patientsFronApi[0].patientId);
        }
    }, [patientsFronApi]);

    const activePatient = patientsFronApi.find((p) => p.patientId === activeId);
    const reports = useReports(1, activeId);
